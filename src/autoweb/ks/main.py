...
from ..tools import log as logger, PcConfig
...
class KuaishouCrawler:
    ...
    def __init__(self, ws: WSClient):
        self.ws = ws
        self.config: PcConfig = ws.config

    async def start(self):
        try:
            global cluster
            cluster = BrowserCluster(max_workers=self.config.BROWSER_MAX_WORKERS)
            self.main()
        except KeyboardInterrupt:
